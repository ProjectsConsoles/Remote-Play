...
| `PS3RP_SIN_VIDEO_S` | `25` | Solo con gstreamer: si no llega NADA de video en estos segundos, cierra el reproductor en vez de quedarse congelado. Si el servidor se relanza solo antes (~10 s), la imagen vuelve sola. |
| `PS3RP_PLAYER=gstreamer_lsfg` | - | **Experimental (2026-09-19).** gstreamer dibujando con Vulkan (`vulkansink`) y la capa de Lossless Scaling del host expuesta dentro del flatpak (acceso de lectura a `~/.local/lib`, `~/.local/share/vulkan`, `~/.config/lsfg-vk` y la carpeta de Lossless Scaling; perfil = `LSFG_PROCESS`, por defecto `PS3`). El ajuste `zoom` no existe en este modo. Si faltan lsfg-vk o su `conf.toml`, cae a `gstreamer` con un AVISO en el log. No toca el modo `gstreamer` normal. |
| `PS3RP_AJUSTE` | `barras` | Solo con gstreamer. La Deck es 16:10 y el video 16:9: `barras` = exacta con franjas negras arriba/abajo; `estirar` = llena la pantalla (~11% mas alta); `zoom` = llena sin deformar, recorta ~5% de cada lado. |
